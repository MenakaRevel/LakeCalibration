...
def Generate_Raven_Lake_rvh_String(catinfo, Raveinputsfolder, Model_Name,lake_out_flow_method,obs_gauge,model_structure):
    ...
    """
    Lake_rvh_file_path = os.path.join(Raveinputsfolder, "Lakes.rvh.tpl")
    ...
    for i in range(0, len(catinfo.index)):
        if catinfo.iloc[i]["HRU_IsLake"] > 0:  ## lake hru
            ...
            if has_obs < 1 or lake_out_flow_method == 'broad_crest':
                ...
                if isinstance(Crewd, str):
                    Lake_rvh_string_list.append(
                    "  :CrestWidth " + Crewd #"{:.4f}".format(Crewd) #str(Crewd)
                    )
                else:
                    Lake_rvh_string_list.append(
...
finalcat_hru_info = pd.read_csv(os.path.join(os.getcwd(),pm.finalcat_hru_info()))
