...
'HYDROGRAPH_CALIBRATION[412]']):
    ...
    # df=df.loc[0:23,:]
    #  DIAG_KLING_GUPTA
    return df.loc[df['observed_data_series'].isin(glist),
    ['DIAG_KLING_GUPTA','DIAG_KLING_GUPTA_DEVIATION','DIAG_R2','DIAG_RMSE']].values #,'DIAG_SPEARMAN']].values
#=====================================================
...
fig, ax = plt.subplots()
sns.boxplot(data=df[['KGE_02KB001','KGE_Crow','KGE_PN','KGE_LM','KGE_NC',
'KGED_02KB001','KGED_Crow','KGED_PN','KGED_LM','KGED_NC',
'R2_02KB001','R2_Crow','R2_PN','R2_LM','R2_NC',
'SRCC_02KB001','SRCC_Crow','SRCC_PN','SRCC_LM','SRCC_NC']])
star=df.loc[df['obj.function'].idxmin(),['KGE_02KB001','KGE_Crow','KGE_PN','KGE_LM','KGE_NC',
'KGED_02KB001','KGED_Crow','KGED_PN','KGED_LM','KGED_NC',
'R2_02KB001','R2_Crow','R2_PN','R2_LM','R2_NC',
'SRCC_02KB001','SRCC_Crow','SRCC_PN','SRCC_LM','SRCC_NC']]
print (star)
...
print (df[['KGE_02KB001','KGE_Crow','KGE_PN','KGE_LM','KGE_NC']].head())
fig, ax = plt.subplots()
ax=sns.boxplot(data=df.loc[:,['KGE_02KB001','KGE_Crow','KGE_PN','KGE_LM','KGE_NC']],
order=['KGE_Crow','KGE_PN','KGE_LM','KGE_NC','KGE_02KB001'])
star=df.loc[df['obj.function'].idxmin(),['KGE_Crow','KGE_PN','KGED_LM','KGED_NC','KGE_02KB001']]
print (star)
ax.scatter(star.index, star.values, marker='*', s=50, color='k')
ax.set_ylabel("$KGE$")
plt.savefig('../figures/'+expname+'/f01-KGE_boxplot_dis.jpg')

plt.close()
plt.clf()

print (df[['KGED_02KB001','KGED_Crow','KGED_PN','KGED_LM','KGED_NC']].head())
fig, ax = plt.subplots()
ax=sns.boxplot(data=df[['KGED_02KB001','KGED_Crow','KGED_PN','KGED_LM','KGED_NC']],
order=['KGED_Crow','KGED_PN','KGED_LM','KGED_NC','KGED_02KB001'])
star=df.loc[df['obj.function'].idxmin(),['KGED_Crow','KGED_PN','KGED_LM','KGED_NC','KGED_02KB001']]
print (star)
ax.scatter(star.index, star.values, marker='*', s=50, color='k')
ax.set_ylabel("$KGED$")
plt.savefig('../figures/'+expname+'/f01-KGED_boxplot_dis.jpg')
