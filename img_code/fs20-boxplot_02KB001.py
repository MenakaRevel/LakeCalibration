...
df=pd.DataFrame(metric, columns=['02KB001','obj.function'])
df['Expriment']=np.array(expriment_name)
print ('='*20+' df '+'='*20)
print (df.head(100))

# df_melted = pd.melt(df[['02KB001','Expriment']], #'obj.function',
# id_vars='Expriment', value_vars=['02KB001']) # ,'obj.function'
# print ('='*20+' df_melted '+'='*20)
# print (df_melted.head(50))

df_melted = pd.DataFrame()
for expname in df['Expriment'].unique():
    print (expname, df[df['Expriment']==expname]['02KB001'])
    df_melted[expname]=df[df['Expriment']==expname]['02KB001'].values
print ('='*20+' df_melted '+'='*20)
print (df_melted.head(50))
...
# order=['02KB001'],hue='Expriment',
# palette=colors, boxprops=dict(alpha=0.9))
ax=sns.boxplot(data=df_melted,palette=colors, boxprops=dict(alpha=0.9))
...
plt.tight_layout()
print ('../figures/paper/fs20-KGE_02KB001_'+datetime.datetime.now().strftime("%Y%m%d")+'.jpg')
plt.savefig('../figures/paper/fs20-KGE_02KB001_'+datetime.datetime.now().strftime("%Y%m%d")+'.jpg')