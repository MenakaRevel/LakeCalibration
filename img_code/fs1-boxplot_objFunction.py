...
'obj.function','mean_ObjLake'])
df['Expriment']=np.array(expriment_name)
print ('='*20+' df '+'='*20)
print (df.head(10))

df_melted = pd.melt(df[['02KB001','Narrowbag',
'Crow','LM','NC','obj.function','mean_ObjLake','Expriment']],
...
order=['obj.function','02KB001','mean_ObjLake','Narrowbag','Crow','LM','NC'],hue='Expriment',
...
for i,expname, color in zip(locs,lexp,colors):
    ...
    print ('='*20+' df_ '+'='*20)
    print (df_.head())
    star=df_.loc[df_['obj.function'].idxmax(),['obj.function','02KB001','mean_ObjLake','Narrowbag','Crow','LM','NC']]#.groupby(['Expriment'])
    # print (star)
...
plt.savefig('../figures/paper/fs1-KGE_boxplot_S0_DiffWave_Dis_'+datetime.datetime.now().strftime("%Y%m%d")+'.jpg')