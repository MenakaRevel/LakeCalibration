...
ax.plot(np.log10(star['Wshd_area'].values),np.log10(star['Initial_CrestWidth'].values), color='grey', label='W~DA',zorder=1)
ax.fill_between(x=np.log10(star['Wshd_area'].values), y1=np.log10(star['Initial_CrestWidth'].values*1.5),
y2=np.log10(star['Initial_CrestWidth'].values*0.5), color='grey',alpha=0.2,zorder=0)
for i,expname in enumerate(lexp):
    ax.plot(np.log10(star['Wshd_area'].values),np.log10(pd.to_numeric(star[expname].values)), #np.log10(
...
for i,group in df_melted[df_melted['Expriment']=='ExpS0b'].groupby('variable'):
    print (i, np.log10(DA_list[group['variable'].values[0]]*1e-6), np.log10(group['value'].values[0]))
    ax.boxplot(np.log10(group['value'].values),positions=[np.log10(DA_list[group['variable'].values[0]]*1e-6)],
...
df_melted2['Wshd_area']=[DA_list[lake]*1e-6 for lake in df_melted2['variable'].values]
print (df_melted2)
slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(np.log10(df_melted2['Wshd_area'].values), np.log10(df_melted2['value'].values))
print (slope, intercept, r_value**2, p_value, std_err)
X=np.linspace(np.min(np.log10(df_melted2['Wshd_area'].values)),np.max(np.log10(df_melted2['Wshd_area'].values)),100)
Y=slope*X+intercept
print (10**intercept,slope)
ax.plot(X, Y, color=colors[0], label='Fitted ExpS0b (%3.2f)'%(r_value**2),zorder=1)

# Perform the curve fitting
popt, pcov = curve_fit(func, df_melted2['Wshd_area'].values, df_melted2['value'].values)
print (popt[0],popt[1])
Y_=popt[1]*np.log10(X)+np.log10(popt[0])
# ax.plot(X, Y_, color=colors[0], label='Fitted ExpS0b (curve_fit)', linestyle='--',zorder=1)
# ax.set_xticks(range(0,4))
# ax.set_xticklabels(range(0,4))

ax.set_ylabel("$log_{10}$$(Lake$ $Crest$ $Width$ $[m])$")
ax.set_xlabel("$log_{10}$$(Watershed$ $Area$ $[km^2])$")
# plt.legend(ncol=8,bbox_to_anchor=(0.5,-0.12),loc='upper center')
#===================
pos = ax.get_position()
print (pos)
pos.y0 = pos.y1 - 0.8*(pos.y1 - pos.y0)      #
# ax.set_position(pos)
#===================
fig.legend(ncol=8,loc='outside upper center')
plt.tight_layout()#pad=0.1)
plt.savefig('../figures/paper/fs2-CresetWidth_wshd_area.jpg')
