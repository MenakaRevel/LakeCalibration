...
df_waterLevel_=read_WaterLevel(exp, 4, best='best_Hogan')
df_diganostics=read_lake_diagnostics(exp, 4)
df_diganostics_=read_lake_diagnostics(exp, 4, best='best_Hogan')
...
#
# create a pdf file
pdfname='../figures/pdf/d02-%s_Hogan_lakes_compare.pdf'%(exp)
with PdfPages(pdfname) as pdf:
    for point in range(len(lake_list)):
        ...
        ax.plot(df_waterLevel.index,df_waterLevel[colWL]-df_waterLevel[colWL].mean(),linestyle='-',linewidth=3,label="observation",color='k')
        # plot WL
        colWA='sub%0d (observed) [m].1'%(sub_list[point])
        axtwin = ax.twinx()
        axtwin.plot(df_waterLevel.index,df_waterLevel[colWA],linestyle='none', marker="o", markersize=4, markerfacecolor="None",markeredgecolor='grey',label="observation [Lake-area]",color='k')
        # plot simulated
        col='sub%0d '%(sub_list[point])
        kge=df_diganostics[(df_diganostics['observed_data_series'].str.contains('CALIBRATION')) & (df_diganostics['filename'].isin([namelist[point]]))][['DIAG_KLING_GUPTA_DEVIATION']].values
        label=exp+' (%5.2f)'%kge
        #KGED(df[col].values,df[colWL].values)
        # df_waterLevel.loc[df_waterLevel.index.month.isin([12,1,2,3]), colWL]=np.nan
        ax.plot(df_waterLevel.index,df_waterLevel[col]-df_waterLevel[col].mean(),linestyle='-',linewidth=1,label=label,color='b')
        ...
        ax.plot(df_waterLevel_.index,df_waterLevel_[col]-df_waterLevel_[col].mean(),linestyle='--',linewidth=0.5,label=label_,color='b')
        ...
        #         colWA='sub%0d (observed) [m].1'%(sub_list[point])
        #         axtwin = ax.twinx()
        #         ax.plot(df.index,df[colWA]-df[colWA].mean(),linestyle='none', marker="o", markersize=4, markerfacecolor="None",markeredgecolor='grey',label="observation [Lake-area]",color='k')
        #     col='sub%0d '%(sub_list[point])
    ...
    d['Keywords'] = 'water level, experiment, '+ exp
