...
for i,expname in enumerate(lexp):
    ...
    points_met = pd.merge(points_df,df_met,on='SubId',how='inner')
...
# ax.set_axis_off()
plt.tight_layout()
plt.savefig('../figures/paper/fs21-map_best_KGE_exp_'+ datetime.datetime.now().strftime("%Y%m%d") +'.jpg', dpi=500) #_summer