...
ax_main.set_extent([-95.16, -74.34, 41.66, 56.86], crs=ccrs.PlateCarree())  # Extent for Canada
ax_main.coastlines()
ax_main.add_feature(cartopy.feature.BORDERS, linestyle=':')
ax_main.add_feature(cartopy.feature.LAND, color='lightgray')
ax_main.add_feature(cartopy.feature.OCEAN, color='lightblue')
ax_main.set_title("Ontario", fontsize=20)

if os.path.exists(path_outline):
    outline = gpd.read_file(path_outline).to_crs("EPSG:4326")
    outline.plot(ax=ax_main, facecolor="none", edgecolor='r', linewidth=1, alpha=0.8)
...
plt.tight_layout()
plt.savefig('../figures/paper/f01-map_study_area_' + datetime.datetime.now().strftime("%Y%m%d") + '.jpg',dpi=500)