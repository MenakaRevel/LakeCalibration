...
#
# create a pdf file
pdfname='../figures/pdf/d09-lake_best_Q_'+datetime.datetime.now().strftime("%Y%m%d")+'.pdf'
#========================================
with PdfPages(pdfname) as pdf:
    for point in range(len(SubIds))[0::]:
        ...
        for cnum, expname in enumerate(lexp):
            ...
            ax.plot(df['date'],df[col],linestyle='-',linewidth=1,label='%s_%02d(%3.2f)'%(expname,num,kged),alpha=0.8,color=colors[cnum]) #,color='b'
            #
    ...
    d = pdf.infodict()
    d['Title'] = 'best Discharge at lake catchments '
