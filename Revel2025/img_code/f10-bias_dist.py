...
ens_num=10
metric=[]
lexp=["V0a","V4d","V5d"] #["V4d","V0a"] #["V0a","V0h","V2e","V4e","V4k","V4d"] #["V0h","V4e","V4k"] #["V0h","V2e","V4e"] #["V0a","V4k","V4d"] #["V0a","V4e","V4k"] #["V0a","V4k","V4d","V4l"]
...
gs = GridSpec(ncols=2, nrows=3, figure=fig) #, height_ratios=[1, 1])

objMet = 'DIAG_KLING_GUPTA' #'DIAG_PCT_BIAS'
objMets = ['DIAG_KLING_GUPTA', 'DIAG_R2', 'DIAG_PCT_BIAS']
for k,objMet in enumerate(objMets):
    points_met = points_df.copy()
    for i,expname in enumerate(lexp):
        objFunction0=1.0
        num = met[expname]
        print (i, expname)
        lq=["./obs/SF_SY_sub%d_%d.rvt"%(subid,subid) for subid in final_cat['SubId'].dropna().unique()]
        df_met=get_df_diagnostics_filename(expname, num, flist=lq, ObjMet=objMet)
        df_met=df_met.loc[:,['SubId',objMet]]
        df_met.rename(columns={objMet:expname}, inplace=True)

        # print (df_met.columns)
        points_met = pd.merge(points_met,df_met,on='SubId',how='inner')

    # points_df['diffKGE']  = points_df[lexp[0]] - points_df[lexp[1]]
    points_met['Lake_cat'] = points_met['SubId'].apply(lambda x: "lake" if x in subids else "non-lake")
    # points_df.rename(coloumns={})
    print (points_met.head())
    ## non-lake
    ax1 = fig.add_subplot(gs[k, 0])
    df_melt=pd.melt(points_met.loc[points_met['Lake_cat']=='lake',lexp],
    value_vars=lexp)
    print (df_melt)
    ...
    # ax1.text(-0.2,0.5,metName[objMet])
    ax1.set_title(
        string.ascii_lowercase[k]+') '+metName[objMet] + ' non-lake', 
        loc='left', 
        fontsize=12)
    ax1.set_xlabel(
        metName[objMet]
    ...
    df_melt=pd.melt(points_met.loc[points_met['Lake_cat']=='non-lake',lexp],
    value_vars=lexp)
    # histogram
    sns.kdeplot(
        data=df_melt,
    ...
    ax2.set_title(
        string.ascii_lowercase[k+1]+') '+metName[objMet] + ' lake', 
        loc='left',
        fontsize=12)
    ax2.set_xlabel(
        metName[objMet]
...
# ax.set_axis_off()
plt.tight_layout()
plt.savefig('../figures/f09-dist_bias_exp_'+lexp[0]+'_'+lexp[1]+'_'+ datetime.datetime.now().strftime("%Y%m%d") +'.jpg', dpi=500) #_summer